...
// Hotel List Tool
const HOTEL_LIST_TOOL: Tool = {
  name: "amadeus_hotel_list",
  description: 
    "Search for hotels in a specific city. Retrieves a list of hotels based on location and optional filters " +
  ...
  inputSchema: zodToJsonSchema(HotelListSchema) as any,
};
...
const HOTEL_SEARCH_TOOL: Tool = {
  ...
  description: 
    "Search for hotel offers (available rooms with pricing) based on location, dates, guests, and other criteria. " +
  ...
  inputSchema: zodToJsonSchema(HotelSearchSchema) as any,
};

// Hotel Offer Tool
const HOTEL_OFFER_TOOL: Tool = {
  name: "amadeus_hotel_offer",
  description: 
    "Get detailed information about a specific hotel offer using its ID. This provides comprehensive details about " +
    "an offer that was found in a previous search result, including updated availability and pricing.",
  inputSchema: zodToJsonSchema(HotelOfferSchema) as any,
};

// Hotel Booking Tool
const HOTEL_BOOKING_TOOL: Tool = {
  name: "amadeus_hotel_booking",
  description: 
    "Book a hotel offer for specified guests. This tool allows you to make an actual reservation using an offer ID " +
    "from a previous search. You'll need to provide guest details and payment information.",
  inputSchema: zodToJsonSchema(HotelBookingSchema) as any,
};
 ...
 */
async function hotelList(params: z.infer<typeof HotelListSchema>): Promise<any> {
  try {
 ...
 */
async function getMockHotelOffers(cityCode: string, checkInDate: string, checkOutDate: string): Promise<any> {
  try {
    ...
    const offers = hotels.map((hotel: any) => {
 ...
 */
async function getMockHotelOfferDetails(offerId: string): Promise<any> {
 ...
 */
async function getMockBookingConfirmation(offerId: string, guests: any[]): Promise<any> {
  return {
    data: {
      id: `BK${Date.now()}`,
 ...
 */
async function hotelSearchImplementation(params: z.infer<typeof HotelSearchSchema>): Promise<any> {
  try {
    ...
    const hotelListParams: z.infer<typeof HotelListSchema> = { 
    ...
    if (params.hotelName) {
      ...
      filteredHotels = hotelListResult.data.filter((hotel: any) => 
    ...
    const hotelIds = hotelsToCheck.map((hotel: any) => hotel.hotelId).filter(Boolean);
 ...
 */
async function makeOffersRequest(params: z.infer<typeof HotelSearchSchema>, hotelIds: string[]): Promise<any> {
  try {
 ...
 */
async function hotelSearch(params: z.infer<typeof HotelSearchSchema>): Promise<any> {
  try {
 ...
 */
async function hotelOffer(params: z.infer<typeof HotelOfferSchema>): Promise<any> {
  try {
 ...
 * Book a hotel offer
 */
async function hotelBooking(params: z.infer<typeof HotelBookingSchema>): Promise<any> {
  try {
 ...
 */
function formatHotelListResults(data: any): string {
 ...
 */
function formatHotelSearchResults(data: any): string {
  if (!data.data || data.data.length === 0) {
 ...
 */
function formatHotelOfferDetails(data: any): string {
  if (!data.data) {
  ...
  if (hotel.contact) {
    result += `📞 Contact\n`;
    if (hotel.contact.phone) result += `Phone: ${hotel.contact.phone}\n`;
    if (hotel.contact.fax) result += `Fax: ${hotel.contact.fax}\n`;
    if (hotel.contact.email) result += `Email: ${hotel.contact.email}\n\n`;
  }
  ...
  if (offer.price) {
    ...
    if (offer.price.taxes && offer.price.taxes.length > 0) {
      result += `Taxes: ${offer.price.taxes.map((tax: any) => 
        `${tax.amount} ${tax.currency} (${tax.included ? 'Included' : 'Not included'})`
      ).join(', ')}\n`;
    }
    result += '\n';
  }
  ...
  if (offer.policies?.cancellations && offer.policies.cancellations.length > 0) {
    ...
    offer.policies.cancellations.forEach((policy: any, index: number) => {
 ...
 */
function formatHotelBookingResults(data: any): string {
  if (!data.data) {
  ...
  if (data.data.associatedRecords && data.data.associatedRecords.length > 0) {
    ...
    data.data.associatedRecords.forEach((record: any) => {
      ...
      case "amadeus_hotel_list": {
        try {
          ...
          if (error?.name === "ZodError" && Array.isArray(error?.errors)) {
            const missingFields = error.errors.map((err: any) => `'${err.path.join(".")}' (${err.message})`).join(", ");
